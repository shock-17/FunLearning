    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-purple-100 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
          ...
          {/* Add Profile Button */}
          {user.kidProfiles.length < 5 && !isAdding && (
            <button
              onClick={() => setIsAdding(true)}
              className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all cursor-pointer border-2 border-dashed border-indigo-300 flex flex-col items-center justify-center"
            >
                ...
                <div className="grid grid-cols-5 gap-4">
                  {avatarOptions.map((avatar) => (
                    <button
                      ...
                      className={`p-2 rounded-lg transition-all ${selectedAvatar === avatar ? 'bg-indigo-100 ring-2 ring-indigo-500' : 'hover:bg-gray-100'}`}
                    >
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                user.kidProfiles.some(
                  (profile) => profile.scores && profile.scores.length > 0,
                ) ? (
                  ...
                  user.kidProfiles
                    .flatMap((profile) =>
                      (profile.scores || []).map((score) => ({
                        ...
                        scoreId:
                          score.id ||
                          `${profile.id}-${score.date}-${Math.random()}`,
                    ...
                    .slice(0, 50) // Limit to last 50 scores
                    .map((scoreEntry) => (
                      <tr key={scoreEntry.scoreId} className="hover:bg-gray-50">
